...
export default defineConfig({
	...
	server: {
		...
		proxy: {
			"/stream": {
				target: `http://${process.env.API_HOST}:${process.env.API_PORT}/sounds/stream`,
	...
	test: {
		...
		coverage: {
			...
			include: [
				...
				"!src/main.tsx",
				"!src/routes.tsx",
				// It'll be great if we test it using e2e tools such as cypress
				// we'll do this in the future :D
